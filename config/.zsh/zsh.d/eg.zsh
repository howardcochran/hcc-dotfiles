...
_eg-push () {
  _arguments \
    '(-b --bypass-modification-check)'{-b,--bypass-modification-test}'[push even in the presence of modified files]' \
    '(--all-branches)--branch:branch name:__git_commits2' \
    '(--branch)--all-branches[push all branches]' \
    '(--all-tags)--tag:tag name:__git_tags' \
    '(--tag)--all-tags[push all tags]' \
    ':repository:__eg_remotes' && ret=0
}
